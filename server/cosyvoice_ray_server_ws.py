...
from conf import COSYVOICE_ROOT_DIR, MATCHA_ROOT_DIR, COSYVOICE_MODEL_DIR, ErrorCode
...
@serve.deployment(num_replicas=10, ray_actor_options={'num_cpus':1, 'num_gpus':0})
...
@serve.deployment(num_replicas=2, ray_actor_options={'num_cpus':1, 'num_gpus':0.8})
@serve.ingress(app)
class CosyVoiceWsRay:
    ...
    async def send_data(self, ws :WebSocket, data: Union[Dict[Any, Any], bytes]):
        try:
    ...
    async def run_task(self, ws: WebSocket):
        ...
        try:
            ...
            print(tb)
            logger.error(f"------------- except An unexpected error occurred: {e}")
            # if ws.client_state == WebSocketState.CONNECTED:
    ...
    async def gen_spk_emb(self, task: dict):
        try:
            ...
            # 参考文本
            prompt_text = await self.tts_front_handle.remote(task['text'],split=False)
...
tts_cosyvoice_app = CosyVoiceWsRay.bind(
