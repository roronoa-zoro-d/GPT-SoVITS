...
@serve.deployment(num_replicas=2, ray_actor_options={'num_cpus':1, 'num_gpus':0.3})
@serve.ingress(app)
class GptSovitsWsRay:
    def __init__(self, spk_emb_handle: DeploymentHandle, tts_front_handle: DeploymentHandle):
        ...
        self.tts_front_handle = tts_front_handle
        self.default_names = self.model.get_spk_names()


    async def save_send_json(self, ws :WebSocket, data: dict):
        await self.update_connect_state()
        flag_send = True
        try:
            ...
            logger.info(f'--------save_send_json detect client close, ws.client_state={ws.client_state} ------')
            flag_send = False
            
        except asyncio.CancelledError:
            logger.info(f'--------save_send_json detect client asyncio.CancelledError , ws.client_state={ws.client_state} ------')
            flag_send = False
        
        return flag_send
    
    async def save_send_json_bytes(self, ws :WebSocket, data: dict, speech_bytes: bytes):
        await self.update_connect_state()
        flag_send = True
        try:
            if ws.client_state == WebSocketState.CONNECTED:
                await ws.send_json(data)
                await ws.send_bytes(speech_bytes)
            else:
                flag_send = False
        except WebSocketDisconnect:
            logger.info(f'--------save_send_json detect client close, ws.client_state={ws.client_state} ------')
    ...
    async def run_task(self, ws: WebSocket):
        ...
        try:
            ...
            while flag_connect:
                ...
                if task_name == 'clone':
                    spk_id = task['spk_id']
                    spk_data = await  self.spk_emb_handle.get_spk_emb.remote(spk_id)
                    if spk_data is None:
                    ...
                    st1 = time.time()
                    texts = await self.tts_front_handle.remote(task['text'])
    ...
    async def gen_spk_emb(self, task: dict):
        try:
            ...
            norm_texts = await self.tts_front_handle.remote(task['text'])
...
tts_app = GptSovitsWsRay.bind(
                SpeakerEmbeddingsRedisRay.bind(), 
                TtsFront.bind(),
            )